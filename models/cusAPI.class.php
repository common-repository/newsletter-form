 ...
 * @api
 * @version 0.4
 * @since 2.0 First time this was introduced into Newsletter Form plugin.
...
class cUsComAPI_NL {
     ...
     * @api
     */
    var $enviroment = cUsNL_API_ENV;
    var $wpApiAccountKey = cUsNL_API_ACC;
    var $wpApiKey = cUsNL_API_AKY;
    ...
    public function getAPICredentials($cUs_email, $cUs_pass) {
        ...
        if( $this->_isCurl() ){
            ...
            curl_setopt($ch, CURLOPT_HTTPHEADER, array('X-ContactUs-Request-URL: ' . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'], 'X-ContactUs-Signature: wpnl|2.0|' . $this->getIP()));
    ...
    public function createCustomer($postData, $credential) {
        ...
        if (!strlen($postData['fname'])) {
            ...
            if( $this->_isCurl() ){
                ...
                $strCURLOPT .= '&Promotion_Code=WPNL';
                $strCURLOPT .= '&Version=wpnl|2.0';
                ...
                curl_setopt($ch, CURLOPT_HTTPHEADER, array(
                    ...
                    'X-ContactUs-Signature: wpnl|2.0|' . $this->getIP()
                ));
    ...
    public function getDefaultDeepLink($cUsNL_API_getFormKeys) {

        if (empty($cUsNL_API_getFormKeys))
            return FALSE;

        //$cUsMC_API_getKeysResult = $this->getFormKeysData($cUs_API_Account, $cUs_API_Key);
        $cUs_jsonKeys = json_decode($cUsNL_API_getFormKeys);
    ...
    public function resetData() {
        ...
        //PLUGIN OPTIONS
        delete_option('cUsNL_settings');
        delete_option('cUsNL_settings_userData');
        delete_option('cUsNL_settings_FORM');
        delete_option('cUsNL_HOME_settings');
        delete_option('cUsNL_settings_step1');
        delete_option('cUsNL_settings_form_key');
        delete_option('cUsNL_settings_inlinepages');
        delete_option('cUsNL_settings_tabpages');
        delete_option('cUsNL_settings_userCredentials');
        delete_option('cUsNL_settings_default_deep_link_view');
        delete_option('cUsNL_settings_form_id');
        delete_option('cUsNL_settings_intro_hints');

        cUsNL_page_settings_cleaner(); //REMOVE CUSTOM SETTINGS BY PAGE
        cUsNL_inline_shortcode_cleaner(); //REMOVE SHORTCODES
